...
class IlluminantColor(Protocol):
    illuminant: color_constants.ILLUMINANTS_TYPE
    observer: color_constants.OBSERVERS_TYPE



def set_observer(color: IlluminantColor, observer: color_constants.OBSERVERS_TYPE) -> None:
    """
    ...
    if observer not in color_constants.OBSERVERS:
        raise InvalidObserverError(color)
    color.observer = observer

def set_illuminant(color: IlluminantColor, illuminant: color_constants.ILLUMINANTS_TYPE) -> None:
    """
...
<colormath.color_objects.XYZColor.apply_adaptation>`.
    ...
    """
    if illuminant not in color_constants.ILLUMINANTS[color.observer]:
